...
std::vector<std::uint8_t> EncodeCancel(const Cancel& cancel) { if (cancel.reason.empty()) throw std::invalid_argument("cancel reason is required"); Writer writer; writer.Bytes(cancel.transfer_id); writer.String(cancel.reason); return writer.Take(); }
Cancel DecodeCancel(std::span<const std::uint8_t> payload) { Reader reader(payload); Cancel cancel{}; auto transfer = reader.Bytes(cancel.transfer_id.size()); std::copy(transfer.begin(), transfer.end(), cancel.transfer_id.begin()); cancel.reason = reader.String(); if (cancel.reason.empty()) throw std::invalid_argument("cancel reason is required"); reader.Finish(); return cancel; }
