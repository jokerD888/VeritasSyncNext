...
std::filesystem::path EngineDirectory() {
  ...
  if (length == 0 || length == buffer.size()) {
    throw std::runtime_error("cannot resolve engine executable path");
  }
  buffer.resize(length);
  return std::filesystem::path(buffer).parent_path();
}

std::filesystem::path WebRtcBridgePath() {
  char* value = nullptr;
  std::size_t length = 0;
  if (_dupenv_s(&value, &length, "VERITASSYNC_WEBRTC_BRIDGE_LIBRARY") == 0 && value != nullptr &&
      length > 1) {
  ...
  std::free(value);
  return EngineDirectory() / "veritassync_webrtc_bridge.dll";
}
...
void RunMockOneWay(const std::string& task_id, const std::filesystem::path& source_root,
  ...
  if (!std::filesystem::is_directory(source_root))
    throw std::invalid_argument("mock source root must exist");
  ...
  for (std::size_t iteration = 0; iteration < kMaximumPumpIterations && !target.TargetIsConverged();
       ++iteration) {
    source.Pump();
    network.PumpUntilIdle();
  }
...
int main(int argc, char** argv) {
  try {
    ...
    std::string mock_source_root, mock_target_root, mock_source_db, mock_target_db,
        mock_task_id = "phase2-mock";
    for (int i = 1; i < argc; ++i) {
      ...
      if (argument == "--ipc-serve") {
        ipc_serve = true;
        continue;
      }
      if (argument == "--mock-one-way") {
        mock_one_way = true;
        continue;
      }
      ...
      if (argument == "--db")
        ...
        scan_task_id = value;
      else if (argument == "--list-conflicts")
        list_conflicts_task_id = value;
      else if (argument == "--resolve-conflict")
        resolve_conflict_id = value;
        ...
        root = value;
      else if (argument == "--mock-source-root")
        mock_source_root = value;
      else if (argument == "--mock-target-root")
        mock_target_root = value;
      else if (argument == "--mock-source-db")
        mock_source_db = value;
      else if (argument == "--mock-target-db")
        mock_target_db = value;
      else if (argument == "--mock-task-id")
        mock_task_id = value;
      else
    ...
    if (ipc_serve) {
      ...
      runtime.SetScanCompletedCallback(
          [&network](const std::string& task_id) { network.LocalChanged(task_id); });
      network.Start();
      runtime.Start();
      veritassync::ipc::IpcService service(database, &pairing, &runtime, &network);
    ...
    if (mock_one_way) {
      if (mock_source_root.empty() || mock_target_root.empty() || mock_source_db.empty() ||
          mock_target_db.empty()) {
    ...
    if (!init_task_id.empty()) {
      if (mode.empty() || role.empty() || root.empty())
        throw std::invalid_argument("--init-task requires --mode, --role, and --root");
    ...
    if (!list_conflicts_task_id.empty()) {
      for (const auto& conflict : database.ListConflicts(list_conflicts_task_id)) {
        std::cout << conflict.conflict_id << "\t" << conflict.state << "\t"
                  << conflict.original_path << "\t" << conflict.conflict_path << "\t"
                  << conflict.winning_version_id << "\n";
      }
    }
    ...
    if (init_task_id.empty() && scan_task_id.empty() && list_conflicts_task_id.empty() &&
        resolve_conflict_id.empty()) {
