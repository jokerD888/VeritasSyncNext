...
VSYNC_TEST(DownloadReceiverMarksOnlyVerifiedWrittenChunksAndCommits) {
  ...
  { veritassync::storage::Database db(db_path); db.ApplyMigrations(); db.CreateTask({"task", "one_way", "target", root.string()});
    ...
    std::vector<std::uint8_t> content(veritassync::protocol::kLogicalChunkSize + 2, 'a'); content.back() = 'd';
    ...
    veritassync::sync::DownloadReceiver receiver(db, id, writer, "file.bin", content.size(), hash, 2);
    const std::vector<std::uint8_t> first(content.begin(), content.begin() + veritassync::protocol::kLogicalChunkSize), second(content.begin() + veritassync::protocol::kLogicalChunkSize, content.end());
    VSYNC_CHECK_THROWS(receiver.AcceptChunk(0, 1, first, veritassync::common::Blake3(first), 2));
    VSYNC_CHECK_THROWS(receiver.AcceptChunk(0, 0, first, veritassync::common::Blake3(second), 2));
    receiver.AcceptChunk(1, veritassync::protocol::kLogicalChunkSize, second, veritassync::common::Blake3(second), 3);
