...
VSYNC_TEST(TrackerClientCreatesSignedInvitationAndRelaysSignals) {
  ...
  capture->responses.push_back({200,
      "INVITE\tABCD-EFGH\nOK\troom-1\t" + std::string(64, 'a') +
      "\tsession-1\t9999999999999\tMEMBERS\nMEMBER\t" + identity.DeviceId() +
      "\t" + identity.Fingerprint() + "\tsource\nEND\n"});
  capture->responses.push_back({200, "OK\n"});
  capture->responses.push_back({200,
      "OK\nSIGNAL\tanswer\tpeer-b\t" + identity.DeviceId() +
      "\tv%3D0%0A\tdata\t0\nEND\n"});
  veritassync::signaling::TrackerClient client("https://tracker.example", identity,
                                                std::move(fake));
  const auto invitation = client.CreateRoom("photos", veritassync::signaling::Topology::kOneWay,
                                             veritassync::protocol::Role::kSource,
                                             veritassync::protocol::Role::kTarget);
  ...
  client.Forward({veritassync::signaling::MessageKind::kOffer, identity.DeviceId(),
                  "peer-b", "v=0\n", "", -1});
