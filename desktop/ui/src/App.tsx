...
import { CreateInvitationDialog, DeviceIdentityCard, JoinInvitationDialog } from "@/components/PairingDialogs";
...
function eventDayKey(timestamp: number) {
  const date = new Date(timestamp);
  return `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, "0")}-${String(date.getDate()).padStart(2, "0")}`;
}

function eventDayLabel(timestamp: number) {
  const key = eventDayKey(timestamp);
  const today = new Date();
  const yesterday = new Date(today);
  yesterday.setDate(today.getDate() - 1);
  if (key === eventDayKey(today.getTime())) return "今天";
  if (key === eventDayKey(yesterday.getTime())) return "昨天";
...
function networkLabel(task: SyncTask) {
  if (!task.enabled) return "已暂停";
  return ({ unpaired: "未配对", offline: "离线", connecting: "连接中", online: "在线", error: "网络错误" } as const)[task.networkStatus];
}
      ...
      <Dialog.Content className="fixed left-1/2 top-1/2 z-50 w-[min(580px,calc(100vw-32px))] -translate-x-1/2 -translate-y-1/2 rounded-2xl border border-line bg-white p-0 shadow-[0_28px_90px_rgba(16,27,51,.28)] focus:outline-none">
        ...
        <form onSubmit={submit} className="space-y-5 px-7 py-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <SelectField label="本机角色" value={mode === "bidirectional" ? "peer" : role} disabled onValueChange={(value) => setRole(value as TaskRole)} options={mode === "bidirectional" ? [{ value: "peer", label: "Peer" }] : [{ value: "source", label: "Source" }]} />
          </div>
...
export function App() {
  ...
  const [preferences, setPreferences] = useState<DesktopPreferences>({ notificationsEnabled: true, colorMode: "dark", trackerUrl: "http://127.0.0.1:8787" });
  ...
  const updateScrollCue = useCallback(() => {
    ...
    if (!viewport) return;
    setCanScrollDown(viewport.scrollHeight - viewport.scrollTop - viewport.clientHeight > 24);
  }, []);
  ...
  const toggleTask = async (task: SyncTask) => { try { if (task.enabled) await engine.pauseTask(task.id); else await engine.resumeTask(task.id); toast.success(task.enabled ? "任务已暂停" : "任务已恢复", { description: task.id }); await refresh(true); } catch (error) { logError(`Task lifecycle change failed for ${task.id}: ${String(error)}`); toast.error("任务状态切换失败", { description: String(error) }); } };
  ...
  return <><div className="app-root" data-theme={resolvedTheme}><Titlebar /><div className="window-shell"><aside className="mica-sidebar"><div className="sidebar-content"><Brand /><nav className="mt-13 grid gap-1">{views.map(({ id, label, icon: Icon }) => <button key={id} className="nav-item" data-active={view === id} onClick={() => setView(id)}><Icon className="size-4" /><span className="flex-1">{label}</span>{id === "conflicts" && taskConflicts.length > 0 ? <span className="rounded-full bg-[#80dbf0] px-2 py-0.5 font-mono text-[10px] font-semibold text-[#112039]">{taskConflicts.length}</span> : null}</button>)}</nav><div className="mt-auto border-t border-white/12 px-2 pt-5"><div className="flex items-center gap-2 text-sm text-[#d6e3f6]"><span className={`status-dot ${engineError ? "problem" : status ? "online" : ""}`} />{engineError ? "需要引擎" : status ? "Engine online" : "连接中"}</div><p className="mt-2 font-mono text-[10px] text-[#8496b4]">IPC / v1 · schema {status?.schemaVersion ?? "—"}</p></div></div></aside>
    <main ref={contentViewport} onScroll={updateScrollCue} className="content-scroll min-w-0 bg-canvas"><div className="content-frame mx-auto max-w-[1540px] px-[clamp(32px,5vw,80px)] py-10"><header className="page-header flex min-h-20 items-center justify-between border-b border-line pb-7"><div><p className="eyebrow">VERITASSYNC NEXT / LOCAL CONTROL</p><h1 className="mt-1 text-4xl font-extrabold tracking-[-.05em] text-ink">{pageTitle}</h1></div><div className="page-actions flex items-center gap-2"><Button variant="ghost" onClick={checkUpdates}><UploadCloud className="size-4" />检查更新</Button><Button size="icon" variant="secondary" aria-label="刷新状态" onClick={() => void refresh()}><RefreshCw className={`size-4 ${refreshing ? "animate-spin" : ""}`} /></Button><JoinInvitationDialog onJoined={async (taskId) => { logInfo(`Joined paired task: ${taskId}`); await refresh(true); }} /><CreateTaskDialog onCreated={async (taskId) => { logInfo(`Task created: ${taskId}`); void notify(preferences, "VeritasSync 已创建任务", `任务 ${taskId} 已由本机引擎接管。`); await refresh(true); }} /></div></header>
      {view === "overview" && <Overview status={status} tasks={tasks} events={events} conflicts={taskConflicts} setView={setView} />}
      {view === "tasks" && <TaskList tasks={tasks} trackerUrl={preferences.trackerUrl} onTrackerUrlChange={async (trackerUrl) => updatePreferences({ ...preferences, trackerUrl })} onScan={scan} onToggle={toggleTask} onRemove={remove} onRefresh={() => refresh(true)} />}
    ...
    </div></main><div className="scroll-cue" data-visible={canScrollDown} aria-hidden="true"><ChevronDown className="size-3.5" />继续滚动</div></div></div><Toaster richColors position="bottom-right" /></>;
}
...
function TaskRow({ task, compact = false, trackerUrl = "", onTrackerUrlChange, onScan, onToggle, onRemove, onRefresh }: { task: SyncTask; compact?: boolean; trackerUrl?: string; onTrackerUrlChange?: (trackerUrl: string) => Promise<void>; onScan?: (task: SyncTask) => void; onToggle?: (task: SyncTask) => void; onRemove?: (task: SyncTask) => void; onRefresh?: () => Promise<void> }) { const canInvite = task.role === "source" || task.role === "peer"; return <div className="data-row"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className="font-semibold text-ink">{task.id}</span><Badge>{roleLabel(task)}</Badge>{task.mode === "bidirectional" && <Badge className="border-[#b9e8df] bg-[#effbf8] text-[#16846a]">双向</Badge>}<Badge className={task.networkStatus === "online" ? "border-[#9cddca] bg-[#edf9f5] text-[#16846a]" : task.networkStatus === "error" ? "border-[#f1bbc1] bg-[#fff1f2] text-[#bd3440]" : ""}>{networkLabel(task)}</Badge>{task.dirty && <Badge>待扫描</Badge>}</div><p className="mt-1 truncate text-sm text-muted">{task.root}</p>{task.networkError && <p className="mt-1 truncate text-xs text-[#d75b68]" title={task.networkError}>{task.networkError}</p>}</div>{compact ? <MoreHorizontal className="size-5 text-[#8a99af]" /> : <div className="flex flex-wrap items-center justify-end gap-1">{canInvite && onTrackerUrlChange && onRefresh && <CreateInvitationDialog task={task} trackerUrl={trackerUrl} onTrackerUrlChange={onTrackerUrlChange} onCreated={onRefresh} />}<IgnoreRulesDialog task={task} /><Button variant="ghost" size="sm" disabled={!task.enabled || task.role === "target"} onClick={() => onScan?.(task)}><RefreshCw className="size-3.5" />扫描</Button><Button variant="ghost" size="sm" onClick={() => onToggle?.(task)}>{task.enabled ? <Pause className="size-3.5" /> : <Play className="size-3.5" />}{task.enabled ? "暂停" : "恢复"}</Button><DeleteTaskButton task={task} onRemove={onRemove} /></div>}</div>; }
...
function TaskList({ tasks, trackerUrl, onTrackerUrlChange, onScan, onToggle, onRemove, onRefresh }: { tasks: SyncTask[]; trackerUrl: string; onTrackerUrlChange: (trackerUrl: string) => Promise<void>; onScan: (task: SyncTask) => void; onToggle: (task: SyncTask) => void; onRemove: (task: SyncTask) => void; onRefresh: () => Promise<void> }) { return <section className="mt-7"><Card className="p-6"><PanelHeading eyebrow="LOCAL ROOTS / PAIRED DEVICES" title="同步任务" />{tasks.length ? <div className="grid gap-2">{tasks.map((task) => <TaskRow key={task.id} task={task} trackerUrl={trackerUrl} onTrackerUrlChange={onTrackerUrlChange} onScan={onScan} onToggle={onToggle} onRemove={onRemove} onRefresh={onRefresh} />)}</div> : <Empty icon={FolderOpen} title="还没有同步任务" detail="新建 Source/Peer 任务，或使用邀请令牌接入已有任务。" />}</Card></section>; }
...
function EventList({ events }: { events: EngineEvent[] }) {
  ...
  return <section className="mt-7"><Card className="p-6"><PanelHeading eyebrow="PERSISTED ENGINE LOG" title="运行事件" />{events.length ? <div className="event-groups">{groups.map(([day, dayEvents]) => <section className="event-day" key={day}><div className="event-day-heading"><span>{eventDayLabel(dayEvents[0].timestamp)}</span><span>{dayEvents.length} 条记录</span></div><div className="divide-y divide-[#ecf0f5]">{dayEvents.map((event) => <EventRow key={event.id} event={event} />)}</div></section>)}</div> : <Empty icon={Clock3} title="暂时没有事件" detail="引擎事件会持久化在本地状态库中。" />}</Card></section>;
}
...
function SettingsPanel({ preferences, onPreferencesChange }: { preferences: DesktopPreferences; onPreferencesChange: (preferences: DesktopPreferences) => Promise<void> }) {
  return <section className="settings-grid mt-7 grid grid-cols-[minmax(0,1fr)_minmax(300px,.6fr)] items-start gap-4">
    <div className="grid gap-4">
      <Card className="p-6"><PanelHeading eyebrow="DESKTOP PREFERENCES" title="桌面体验" /><div className="grid gap-2"><Setting icon={Moon} title="颜色模式" detail="默认使用深色工作台；也可以切换为浅色或跟随 Windows。" enabled control={<div className="theme-options">{([{ value: "dark", label: "深色", icon: Moon }, { value: "light", label: "浅色", icon: Sun }, { value: "system", label: "系统", icon: Monitor }] as const).map(({ value, label, icon: Icon }) => <button key={value} type="button" data-active={preferences.colorMode === value} onClick={() => void onPreferencesChange({ ...preferences, colorMode: value })}><Icon className="size-3.5" />{label}</button>)}</div>} /><Setting icon={MonitorCog} title="窗口状态" detail="下次启动时恢复上次的位置、尺寸和最大化状态。" enabled /><Setting icon={Bell} title="系统通知" detail="同步完成、冲突和需要注意的错误会显示 Windows 通知。" enabled={preferences.notificationsEnabled} control={<button type="button" role="switch" aria-checked={preferences.notificationsEnabled} onClick={() => void onPreferencesChange({ ...preferences, notificationsEnabled: !preferences.notificationsEnabled })} className={`relative h-6 w-11 rounded-full transition ${preferences.notificationsEnabled ? "bg-brand" : "bg-[#b8c4d3]"}`}><span className={`absolute top-1 size-4 rounded-full bg-white shadow transition ${preferences.notificationsEnabled ? "left-6" : "left-1"}`} /></button>} /><Setting icon={CircleHelp} title="诊断日志" detail="桌面壳和前端错误会写入本地诊断日志，不写入同步数据库。" enabled /></div></Card>
      <AiProviderSettings />
    </div>
    <div className="grid gap-4">
      <Card className="p-6"><PanelHeading eyebrow="DEVICE IDENTITY" title="设备与 Tracker" /><DeviceIdentityCard /><label className="mt-4 grid gap-2 text-sm font-semibold text-muted">默认 Tracker 地址<input type="url" value={preferences.trackerUrl} onChange={(event) => void onPreferencesChange({ ...preferences, trackerUrl: event.target.value })} className="pairing-input font-mono" /><small className="font-normal leading-5">本机 HTTP 只用于开发；非本机地址必须使用 HTTPS。</small></label></Card>
      <Card className="p-6"><PanelHeading eyebrow="SECURITY" title="数据边界" /><div className="space-y-4 text-sm leading-6 text-muted"><p>桌面 UI 通过版本化命名管道请求 Engine，不直接读取 SQLite 数据库或同步目录。</p><p className="rounded-xl border border-[#dce8fa] bg-[#f6f9ff] p-4 text-[#516d94]"><ShieldCheck className="mr-2 inline size-4 text-brand" />系统级目录选择、更新验证和通知均由 Tauri 原生能力完成。</p><p className="rounded-xl border border-[#dce8fa] bg-[#f6f9ff] p-4 text-[#516d94]"><Bot className="mr-2 inline size-4 text-brand" />AI 只能提出候选忽略规则；Engine 会独立校验、预览风险，并在你确认后原子写入。</p></div></Card>
    </div>
  </section>;
}
