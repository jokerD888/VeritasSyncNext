...
function Brand() {
  return <div className="flex items-center gap-3 px-2"><span className="brand-mark-shell"><img src={veritasSyncMark} alt="VeritasSync" className="size-10 rounded-full" /></span><div className="leading-tight"><div className="text-sm font-extrabold tracking-[.13em] text-white">VERITAS</div><div className="text-sm font-extrabold tracking-[.13em] text-[#85dff0]">SYNC</div></div></div>;
}
...
function Titlebar() {
  ...
  return <div className="titlebar" data-tauri-drag-region onDoubleClick={() => void window.toggleMaximize().catch(() => undefined)}>
    <div className="flex items-center gap-2" data-tauri-drag-region><img src={veritasSyncMark} alt="" className="titlebar-mark size-5 rounded-full" /><span className="font-semibold tracking-[-.015em]">VeritasSync Next</span><span className="titlebar-build">LOCAL WORKSPACE</span></div>
...
function CreateTaskDialog({ onCreated }: CreateTaskDialogProps) {
  ...
  return <Dialog.Root open={dialogOpen} onOpenChange={setDialogOpen}>
    <Dialog.Trigger asChild><Button className="liquid-cta"><Plus className="size-4" /> 新建任务</Button></Dialog.Trigger>
    <Dialog.Portal>
...
export function App() {
  ...
  return <><div className="app-root" data-theme={resolvedTheme}><Titlebar /><div className="window-shell"><div className="ambient-field" aria-hidden="true"><span className="ambient-blob ambient-blob-one" /><span className="ambient-blob ambient-blob-two" /><span className="ambient-blob ambient-blob-three" /><span className="ambient-caustic" /></div><aside className="mica-sidebar"><div className="sidebar-content"><Brand /><nav className="mt-13 grid gap-1">{views.map(({ id, label, icon: Icon }) => <button key={id} className="nav-item" data-active={view === id} onClick={() => setView(id)}><Icon className="size-4" /><span className="flex-1">{label}</span>{id === "conflicts" && taskConflicts.length > 0 ? <span className="rounded-full bg-[#80dbf0] px-2 py-0.5 font-mono text-[10px] font-semibold text-[#112039]">{taskConflicts.length}</span> : null}</button>)}</nav><div className="engine-status-panel mt-auto"><div className="flex items-center gap-2 text-sm text-[#d6e3f6]"><span className={`status-dot ${engineError ? "problem" : status ? "online" : ""}`} />{engineError ? "需要引擎" : status ? "Engine online" : "连接中"}</div><p className="mt-2 font-mono text-[10px] text-[#8496b4]">IPC / v1 · schema {status?.schemaVersion ?? "—"}</p></div></div></aside>
    <main ref={contentViewport} onScroll={updateScrollCue} className="content-scroll min-w-0 bg-canvas"><div className="content-frame mx-auto max-w-[1540px] px-[clamp(32px,5vw,80px)] py-10"><header className="page-header flex min-h-20 items-center justify-between border-b border-line pb-7"><div><p className="eyebrow">VERITASSYNC NEXT / LOCAL CONTROL</p><h1 className="mt-1 text-4xl font-extrabold tracking-[-.05em] text-ink">{pageTitle}</h1></div><div className="page-actions flex items-center gap-2"><Button variant="ghost" onClick={checkUpdates}><UploadCloud className="size-4" />检查更新</Button><Button size="icon" variant="secondary" aria-label="刷新状态" onClick={() => void refresh()}><RefreshCw className={`size-4 ${refreshing ? "animate-spin" : ""}`} /></Button><CreateTaskDialog onCreated={async (taskId) => { logInfo(`Task created: ${taskId}`); void notify(preferences, "VeritasSync 已创建任务", `任务 ${taskId} 已由本机引擎接管。`); await refresh(true); }} /></div></header>
